...
from typing import Any, Callable, Iterable, Protocol, TextIO, Type, TypeVar
...
def _parse_value2(field_type: Type, stream: bytes) -> Any:
    """Parse a field value from a byte stream."""
    # If target type is string or bytes just convert to a hex representation.
    # Note that this is done for `bytes` as well, so they are stored as string
    # representations as well.
...
def parse_struct(
    ...
    for field in layout:
        ...
        kwargs[field.name] = _parse_value2(field.type, header_bytes[start:end])
        start = end
    return header_type(**kwargs)
